...
import { getOAuthHeaders, prepareApiRequest } from '../src/utils/teleport/api.ts'
...
const CANDIDATES: Array<{ path: string; betas: string[] }> = [
  ...
  { path: '/v1/agents', betas: ['', 'managed-agents-2026-04-01', 'agents-2026-04-01'] },
  { path: '/v1/vaults', betas: ['', 'managed-agents-2026-04-01', 'vaults-2026-04-01'] },
...
async function probe(
  ...
  for (const beta of candidate.betas) {
    ...
    console.log(`  ${verdict.padEnd(6)} ${padded} ${betaPad}  ${body.slice(0, 110)}`)
  }
}

async function main(): Promise<void> {
  console.log('=== Probe subscription OAuth bearer against /v1/* candidates ===\n')
