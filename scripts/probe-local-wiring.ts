...
async function main() {
  ...
  for (const [k, label] of ADVERSARIAL_KEYS) {
    ...
    probe(`validateKey rejects ${label}`, rejected, JSON.stringify(k.slice(0, 30)))
  }
  ...
  probe(
    ...
    validatePermissionRule('VaultHttpFetch(github-token)', 'allow').valid === false,
    'C1 host binding',
  )
  probe(
  ...
  const warnings = filterInvalidPermissionRules(settingsData, '/test/probe.json')
  probe(
  ...
  const errMsg = scrubAxiosError(new FakeAxiosError(`failed: ${SECRET} not authorized`), forms)
  probe(
  ...
  try {
    ...
    const r1 = await LocalMemoryRecallTool.call(
      { action: 'fetch', store: 'attack-store', key: 'big1', preview_only: false },
      turnCtx,
    )
    const r2 = await LocalMemoryRecallTool.call(
      { action: 'fetch', store: 'attack-store', key: 'big2', preview_only: false },
      turnCtx,
    )
    const r3 = await LocalMemoryRecallTool.call(
      { action: 'fetch', store: 'attack-store', key: 'big3', preview_only: false },
      turnCtx,
    )
    probe(
  ...
  const mctx = {
    getAppState: () => ({
      toolPermissionContext: {
        ...
        alwaysAllowRules: { user: [], project: [], local: [], session: [], cliArg: [] },
        alwaysDenyRules: { user: [], project: [], local: [], session: [], cliArg: [] },
        alwaysAskRules: { user: [], project: [], local: [], session: [], cliArg: [] },
