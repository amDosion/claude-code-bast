...
} from '../../services/providerRegistry/switcher.js'
...
export const callProviders: LocalJSXCommandCall = async (
  onDone,
  _context,
  args,
) => {
  ...
  if (parsed.action === 'list') {
    ...
    onDone(
      `${providers.length} provider(s) registered. Active: ${activeId ?? 'none (Anthropic direct)'}`,
      { display: 'system' },
    )
  ...
  if (parsed.action === 'use') {
    ...
    onDone(
      `To use ${id}: add the shell exports to your profile and restart Claude Code.`,
      { display: 'system' },
    )
    return React.createElement(ProviderView, {
      mode: 'used',
