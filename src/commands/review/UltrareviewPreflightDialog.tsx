 ...
 */
export function UltrareviewPreflightDialog({
  billingNote,
  onConfirm,
  onCancel,
  ...
  const handleSelect = useCallback(
    (value: string) => {
      if (value === 'proceed') {
        setIsLaunching(true)
        void onConfirm(abortControllerRef.current.signal).catch(() =>
  ...
  const displayNote =
    billingNote ?? 'This run may incur additional cost.'

  return (
    <Dialog
      title="Ultrareview — additional cost"
