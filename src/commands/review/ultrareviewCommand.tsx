...
export const call: LocalJSXCommandCall = async (onDone, context, args) => {
  ...
  if (preflight?.action === 'blocked') {
    ...
    const message = preflight.billing_note ?? 'Ultrareview is currently unavailable.';
      ...
      <UltrareviewPreflightDialog
        ...
        onConfirm={async signal => {
          await launchAndDone(
            ...
            context,
            onDone,
            preflight.billing_note ? ` ${preflight.billing_note}` : ' This review bills as Extra Usage.',
            signal,
          );
        }}
        onCancel={() => onDone('Ultrareview cancelled.', { display: 'system' })}
